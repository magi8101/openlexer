...
impl LexerSpec {
    ...
    pub fn parse(input: &str) -> Result<Self> {
        ...
        spec.start_conditions.insert("INITIAL".to_string(), StartConditionType::Inclusive);
        ...
        for line in input.lines() {
            ...
            if trimmed.contains("::=") {
                ...
                if indicator_examples.len() < 2 {
                    indicator_examples.push(format!("'{}' (BNF production rule)", trimmed.chars().take(50).collect::<String>()));
                }
            }
            ...
            if trimmed.starts_with('|') && !trimmed.contains('{') {
                ...
                if indicator_examples.len() < 2 {
                    indicator_examples.push(format!("'{}' (grammar alternative)", trimmed.chars().take(40).collect::<String>()));
                }
            }
            
            // Check for %token declarations (these are parser declarations)
            if trimmed.starts_with("%token") || trimmed.starts_with("%type") || 
               trimmed.starts_with("%left") || trimmed.starts_with("%right") ||
               trimmed.starts_with("%nonassoc") || trimmed.starts_with("%start") {
                grammar_indicators += 2;
                if indicator_examples.len() < 2 {
                    indicator_examples.push(format!("'{}' (parser declaration)", trimmed.chars().take(40).collect::<String>()));
                }
            }
            
            // Check for grammar symbols in angle brackets like <EOF>, <IDENTIFIER>
            if trimmed.contains("<EOF>") || trimmed.contains("<IDENTIFIER>") || 
               trimmed.contains("<INTEGER_LITERAL>") || trimmed.contains("<STRING_LITERAL>") {
                grammar_indicators += 1;
            }
        }
    ...
    fn parse_definitions(&mut self, input: &str) -> Result<()> {
        ...
        for line in input.lines() {
            ...
            if trimmed.starts_with("%s") {
                ...
                for name in names {
                    self.start_conditions.insert(name.to_string(), StartConditionType::Inclusive);
                }
                continue;
            }
            ...
            if trimmed.starts_with("%x") {
                let names = trimmed[2..].split_whitespace();
                for name in names {
                    self.start_conditions.insert(name.to_string(), StartConditionType::Exclusive);
                }
                continue;
            }
    ...
    fn parse_rule_multiline(&self, lines: &[&str], start_idx: usize) -> Result<(LexerRule, usize)> {
        ...
        let (pattern_raw, action_str, extra_lines) = split_pattern_action_flex(remaining, lines, start_idx, line_number)?;
    ...
    fn expand_definitions(&self, pattern: &str) -> String {
        ...
        while let Some(c) = chars.next() {
            if c == '\\' {
                ...
                if found_close && !name.is_empty() && name.chars().all(|c| c.is_alphanumeric() || c == '_') {
...
fn collect_brace_action(first_part: &str, all_lines: &[&str], start_idx: usize) -> Result<(String, usize)> {
...
"#;
        let spec = LexerSpec::parse(input).unwrap();
        assert_eq!(spec.start_conditions.get("COMMENT"), Some(&StartConditionType::Exclusive));
        assert_eq!(spec.start_conditions.get("STRING"), Some(&StartConditionType::Inclusive));
        assert_eq!(spec.start_conditions.get("INITIAL"), Some(&StartConditionType::Inclusive));
...
"#;
        let spec = LexerSpec::parse(input).unwrap();
        assert_eq!(spec.rules[0].action, RuleAction::Begin("COMMENT".to_string()));
        assert_eq!(spec.rules[1].action, RuleAction::Begin("INITIAL".to_string()));
    }
...
"#;
        ...
        assert!(spec.rules.len() >= 5, "Expected at least 5 rules, got {}", spec.rules.len());
