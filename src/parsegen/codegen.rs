...
fn generate_c_minimal(table: &ParsingTable, grammar: &Grammar) -> Result<String> {
    ...
    if let Some(epilogue) = &grammar.epilogue {
        c.push_str("/* User epilogue from grammar trailer */\n");
        c.push_str(epilogue);
        c.push_str("\n\n");
    }
...
fn generate_java_minimal(table: &ParsingTable, grammar: &Grammar) -> Result<String> {
    ...
    c.push_str("/** Parser generated by OpenLexer\n");
    c.push_str(" *\n");
    c.push_str(" * File Organization:\n");
    c.push_str(" *   - This file can be standalone (has inline lexer)\n");
    ...
    // Inline lexer state (used only if no external Lexer.class available)
    c.push_str("    static String src; static int pos; static int yylval;\n");
    c.push_str("    static Object externalLexer = null;\n\n");

    // Inline tokenizer
    let number_tid = term_ids.get("NUMBER").or_else(|| term_ids.get("number")).or_else(|| term_ids.get("NUM"));
    c.push_str("    static int yylex() {\n");
    c.push_str("        // Try external Lexer first (if compiled together)\n");
    c.push_str("        if (externalLexer != null) {\n");
    c.push_str("            try {\n");
    c.push_str("                Object tok = externalLexer.getClass().getMethod(\"nextToken\").invoke(externalLexer);\n");
    c.push_str("                String typeStr = tok.getClass().getField(\"text\").get(tok).toString();\n");
    c.push_str("                int typeVal = (int) tok.getClass().getField(\"type\").getInt(tok);\n");
    c.push_str("                yylval = 0;\n");
    c.push_str("                try { yylval = Integer.parseInt(typeStr); } catch (Exception ignored) {}\n");
    c.push_str("                return typeVal;\n");
    c.push_str("            } catch (Exception e) { externalLexer = null; }\n");
    c.push_str("        }\n");
    c.push_str("        // Fall back to inline lexer\n");
    ...
    // Main - attempts to use external Lexer if available  
    ...
    c.push_str("        }\n");
    c.push_str("        // Try to use external Lexer.class if available\n");
    c.push_str("        try {\n");
    c.push_str("            Class<?> lexerClass = Class.forName(\"Lexer\");\n");
    c.push_str("            externalLexer = lexerClass.getConstructor(String.class).newInstance(expr);\n");
    c.push_str("            System.out.println(\"[Using external Lexer.class]\");\n");
    c.push_str("        } catch (Exception e) {\n");
    c.push_str("            System.out.println(\"[Using inline lexer]\");\n");
    c.push_str("        }\n");
