...
use crate::parsegen::grammar::{Grammar, Symbol, Assoc, Rule};
...
impl ParsingTable {
    pub fn build(grammar: &Grammar) -> Result<Self> {
            ...
            if let Some(trans) = transitions.get(&state.id) {
                for (sym, next_state_id) in trans {
                    if augmented.tokens.contains(sym) || sym == "$" {
                        // Shift
                        add_action(&mut table, state.id, sym, Action::Shift(*next_state_id), &augmented);
                    } else {
                        // Goto
                        table.goto.entry(state.id).or_default().insert(sym.clone(), *next_state_id);
                    }
                }
            }
                    ...
                    if item.rule_index == 0 {
                        ...
                        if let Some(follow) = ff.follow.get(&rule.lhs) {
                            for term in follow {
                                ...
                                add_action(&mut table, state.id, term, Action::Reduce(item.rule_index - 1), &augmented);
                            }
                        }
                    }
                }
        ...
        if table.shift_reduce_conflicts > 0 || table.reduce_reduce_conflicts > 0 {
            eprintln!("Parser has {} shift/reduce and {} reduce/reduce conflicts",
...
fn add_action(table: &mut ParsingTable, state_id: usize, sym: &str, action: Action, grammar: &Grammar) {
...
fn build_lr0_collection(grammar: &Grammar) -> Result<(Vec<State>, HashMap<usize, HashMap<String, usize>>)> {
    ...
    let initial_core: BTreeSet<Item> = [Item { rule_index: 0, dot: 0 }].into();
    let initial_set = closure(grammar, &initial_core);
    
    let start_state = State { id: 0, items: initial_set.clone() };
    ...
    while let Some(state_id) = work_list.pop() {
        ...
        for sym in symbols {
            ...
            if next_core.is_empty() { continue; }
            ...
            let next_state_id = if let Some(&id) = state_map.get(&next_set) {
                ...
                let id = states.len();
                states.push(State { id, items: next_set.clone() });
                state_map.insert(next_set, id);
                work_list.push(id);
                id
            };
            
            transitions.entry(state_id).or_default().insert(sym.clone(), next_state_id);
        }
    }
        ...
        if item.dot < grammar.rules[item.rule_index].rhs.len() {
             ...
             if let Symbol::NonTerminal(nt) = sym {
                 ...
                 for (i, rule) in grammar.rules.iter().enumerate() {
                     if &rule.lhs == nt {
                         let new_item = Item { rule_index: i, dot: 0 };
